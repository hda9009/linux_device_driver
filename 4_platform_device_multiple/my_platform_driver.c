...
struct platform_driver my_platform_driver[MAX_DEVICES] = {
    [DEVICE_ID_1] = {
...
static int __init platform_driver_init(void)
{
    ...
    for (device_number = 0; device_number < MAX_DEVICES; device_number++)
    {
        ...
        platform_driver_register(&my_platform_driver[device_number]);
        // platform_register_drivers(&my_platform_driver, MAX_DEVICES);

        pr_info("Platform Driver Registered [name] = [%s]\n", my_platform_driver[device_number].driver.name);
    }
...
static void __exit platform_driver_exit(void)
{
    ...
    for (device_number = 0; device_number < MAX_DEVICES; device_number++)
    {
        platform_driver_unregister(&my_platform_driver[device_number]);
    }
